...
exports.getDate = function(cb) {
  cb(null, moment().tz("America/Vancouver").format("ddd, MMMM Do"));
};

exports.getDateTomorrow = function(cb) {
  var date = moment().tz("America/Vancouver").add('d', 1).format("ddd, MMMM Do");
  cb(null, date);
};
...
exports.getMonth = function(cb) {
  var reply = "";
  if (this.message.words.indexOf("next") != -1) {
    reply = moment().tz("America/Vancouver").add('M', 1).format("MMMM");
  } else if (this.message.words.indexOf("previous") != -1) {
    reply = moment().tz("America/Vancouver").subtract('M', 1).format("MMMM");
  } else if (this.message.words.indexOf("first") != -1) {
    reply = "January";
  } else if (this.message.words.indexOf("last") != -1) {
    reply = "December";
  } else {
    reply = moment().tz("America/Vancouver").format("MMMM");
  }
  cb(null, reply);
};
